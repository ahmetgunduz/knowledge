...
We say A is a _δ-approximation algorithm_ if Ratio~A~(x) ≤ δ ∀ feasible x.
...
- Book "Design and Analysis of Randomized Algorithms", chapter "7.2 Relaxation to Linear Programming"
...
Let A be a randomized algorithm for an optimization problem, and let OPT be an optimal algorithm. A is a __randomized E[δ]-approximation algorithm__ if for every feasible input x it a) returns a feasible solution and b) E[Ratio~A~(x)] ≤ δ.

With the same context, A is a __randomized δ-approximation algorithm__ if b) is replaced by Pr[Ratio~A~(x) ≤ δ] ≥ 1/2.  Note that such an algrithm can be repeated arbitrary many times to make Pr[Ratio~A~(x) ≤ δ] arbitrarily close to 1.
