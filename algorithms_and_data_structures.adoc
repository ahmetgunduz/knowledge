...
A _dynamic connectivity_ structure is a data structure that dynamically maintains information about the connected components of a graph.

A _fully dynamic_ structure is one where you can do inserts and deletes, a _partially dynamic_ structure is one where you can do inserts or deletes, an _incremental_ structure is one where you can only do only insertions, a _decremental_ structure is one where you only can do deletes.  Incremental dynamic connectivity corresponds to <<disjoint_set>>.
...
_connectivity_(v:vertex, w:vertex): Returns true if v and w are connected

_connectivity_(): Returns true if the graph is connected. Is more or less equivalent to connectivity(v,w) in terms of asympotic analysis.


Implementations / Overview of results. n = |V|, m = |E|.

- trees: O(log n) amortized for all ops using <<link_cut_tree>>

- trees, decremental: O(1) amortized *to-do*
...
Let A be an consistent algorithm for an optimization problem, and OPT an optimal algorithm.  The _approximation ratio_ of A on x is defined as Ratio~A~(x) = cost(A(x)) / cost(OPT(x)) for minimization problems, and the reciproce of it for maximization problems.
...
See also <<randomized_algs_for_optimization_problems>>.

Solution techniques:

- Many discrete optimization problems can be reduced to <<ILP>>.
...
The _standard form_ (aka _general form_, or _primal form_ (see LP duality)) is defined as follows:
...
An implementation of the <<dynamic_connectivity>> ADT. *to-do* but offers more ops, right?
...
*to-do* why not store the represented tree in the obvious way? Because it might be a path and then many ops would be O(n) but we want O(log n)?

Note that the represented tree might not be an binary tree.  Imagine an Euler Tour of the represented tree as a curved line, make a mark on the line every time you visit a node, then pull that line straight.  The marks on the straight line define the leaves of an BBST.  I.e. the root of the represented tree corresponds to the leftmost and and the rightmost leaf in the BBST.  Each node of the represented tree has a pointer to the leaf in the BBST representing its first visit, and likewise a pointer to its last visit.

*to-do* Erik said "remember that we are dealing really with forests". Do we?
...
See also the <<dynamic_connectivity>> ADT, and the list of concrete data structures listed there.

Algorithms for finding connected components (in undirected graphs):

- Using a fast <<disjoint_set>> data structure, put each vertex into its own set. Then iterate over all edges, and join the two sets associated with the two end points.  Each remaining set represents a connected component.  Runs in O(V+E·α(V)) time and needs O(V) auxillary space, assuming an disjoint set forest is used.  α is the very slowly growing inverse function of the Ackermann function.
