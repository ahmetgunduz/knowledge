...
See also <<interval_tree>> and <<segment_tree>>.
...
Augments a binary search tree to store a set of intervals.  In case a BBST is augmented, each operation can be done in O(log n) time.  See also <<range_tree>> and <<segment_tree>>.
...
See also <<interval_tree>> and <<range_tree>>.
