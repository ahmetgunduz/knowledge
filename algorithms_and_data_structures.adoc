...
Online:: An online algorithm is one that can process its input piece-by-piece in a serial fashion, i.e. the algorithm does not need to see the entire input from the beginning. See <<online_algorithms>>.
...
An online algorithm is _k-competitive_ if there exists a constant k such that its cost is at most k times the cost of an optimal offline algorithm.  The factor k is called the _competitive ratio_.
