...
Intuitively: While there are vertixes left, pick a source (i.e. a vertex with in-degree 0), append it to output sequence, remove it from graph.  If any vertices are left, the graph contains at least one cycle.
...
*to-do*: Could we also do the same, but with sinks instead sources?
