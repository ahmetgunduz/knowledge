...
Intuitively the algorithm is: Starting at the source vertex, a spanning tree is build, prefering to grow the tree in depth rather than breadth. A stack maintains the neighbors of the vertices already in the spanning tree.  When the current vertex adds a neighbor to the stack he means `I want that neighbor as my spanning tree child'.  Due to the stacks FILO policy, that `takes away' that neighbor from any spanning tree anchestor who also wanted that neighbor as its child.  Thus the stack can contain a vertex multiple times.  Once poped, the top most occurence wins, i.e. is added to the spanning tree.  The other occurences naturally are still in the stack, but now the vertex is already part of the spanning tree.  To avoid adding it to the spanning tree again when popped again, a `visited' vertex attribute is needed.


Basic algorithm 1 for a connected graph:
--------------------------------------------------
DFS-visit(G:graph, root:vertex):
  // 'visited' means part of the spanning tree. Not needed for trees.
  root.visited = false
  create stack and push root
  while stack is not empty:
    current = stack.pop()
    if !current.visited: // i.e. if not already part of spanning tree
      // Add current to spanning tree. Note that current might
      // occur again further below in the stack.
      current.visited = true
      for each neighbor:
        // The if is not required, but it would be silly to omit it.
        if !neighbor.visited:   
          // Announce that the current vertex wants neighbor as its spanning tree
          // child.  That takes 'take away' that neighbor from any anchestor of the
          // current vertex who also wanted it as its child.
          stack.push(neighbor)  
--------------------------------------------------
