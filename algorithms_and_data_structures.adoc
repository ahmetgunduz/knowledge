...
A binary tree T having the min heap property and having the additional porperty that its in-order traversal delivers a given array A.  In other words, each node has the min heap property and the property that when it corresponds to the element A[i], its left subtree is a cartesian tree of A[<i], and its right subtree is a cartesion tree of A[>i].
...
For each array type, we store a lookuptable which solves RMQ on that array type in O(1). (*to-do* why exactly uses Demain the naive lookuptable instead the power of two variant?)
...
- With preprocessing: Variants of <<trie>>s: <<suffix_tree>>, <<suffix_array>>
