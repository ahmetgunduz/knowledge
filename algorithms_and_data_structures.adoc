...
- forest: O(log n) amortized for all ops using <<link_cut_tree>> or <<euler_tour_tree>>.

- forest, decremental: O(1) amortized using <<micro_macro_tree_decomposition>>
...
An implementation of the <<dynamic_connectivity>> ADT, for a (undirected) graph being constrained to a forest.  Each of the represented trees is stored using an Euler tour tree.  O(log n) time for all query and update operations and O(n) space.
...
Note that the represented tree might not be an binary tree.  Also note that depending on the context, the represented tree is an undirected, i.e. there is no parent child direction.

The core idea of an ET tree is to store the Euler tour of the represented tree. Recall that strictly speaking tree doesn't have an Euler Tour.  So figuratively double each edge in the represented tree for a Euler tour to become possible.

Aribitrarily pick any node of the represented (undirected) tree as its root.  Instead root you might also say as its representative, as we did in <<disjoint_set>>.  Imagine the Euler Tour as a curved line.  Make a mark on the line every time you visit a node, then pull that line straight.  The marks on the straight line define the leaves of a complete balanced binary tree.  I.e. the (arbitrarily picked) root of the represented tree corresponds to the leftmost and and the rightmost leaf in that balanced binary tree.  Note that the subtree rooted at v corresponds to a consecutive sequence in the Euler Tour, starting at v's first visit and ending at v's last visit.
...
_link(u:vertex, v:vertex)_:  Adds edge {u, v}. Make u and v the representative of their respective represented tree.  Concatenate u's ET tree, v' Euler toor tree, u.  Analysis: O(log n), see cut and setrepresentative.

_Aggregate over subtree rooted at v_: Example for aggregate queries would be sum, min, max etc.  Do a range query in the leaves [firstvisit(v), lastvisit(v)], see also the notes about how to store the Euler-tour tree above. *to-do* this only makes sense when it's a directed graph, which by problem statement we don't have. ok, if we use ET trees in another context than dynamic connectivity for forest.
...
- Much better than MIT's: http://web.stanford.edu/class/archive/cs/cs166/cs166.1146/lectures/04/Small04.pdf plus !! bugfixes: http://web.stanford.edu/class/archive/cs/cs166/cs166.1146/lectures/05/Small05.pdf
