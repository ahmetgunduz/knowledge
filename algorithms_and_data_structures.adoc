...
Is a greedy algrithm that solves the <<maximum_flow_problem>>.
...
A _flow network_ (aka _transportation network_) is a directed graph where each edge has a _capacity_ and a _flow_ which can not exceed the capacity.  The amount of flow into a node must equal the amount of flow out of it, unless the node is a _source_ or a _sink_.  An __s-t edge cut__ is an edge cut such that one of the resulting component contains the source and the other component contains the sink.  The __edge-cut capacity__ is the sum of the capacities on the cut edges.
...
_Maximum flow problems_ involve finding the maximum flow in a flow network.  It can be seen as a special case of more complex problems, e.g. the circulation problem.   The _max-flow min-cut theorem_ states that the maximum flow is equal to the minimum capacity over all possible s-t edge cuts.
...
Solutions of max-flow: _Ford–Fulkerson algorithm_ +O(E*f)+ for the case where capacities are integers, whereas f is the maximum flow. See <<ford_fulkerson>>.
...
// compile-command: "asciidoc -a toc -a icons algorithms_and_data_structures.adoc"
// End:
