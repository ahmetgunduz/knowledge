...
Analysis: O(k) query time and O(n) space, see fractional cascading.  Note that due to the way we cross linked the arrays, the arrays corresponding to rays starting above q.z are not looked at in any way, i.e. they cost us O(0), which is exactly what we need.
