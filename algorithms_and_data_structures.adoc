...
| (1D) binary indexed tree | O(lg n)           | O(n log n)      | O(n)            | *to-do*
|=====
...
Data structure: Use kind of a layered range tree, where the first level sorts points by their x coordinate. Each node conceptually stores a data structure for an yz <<2D_dominance_query>>.  Recall that the query time in such a 2D dominance query data structure is dominated by the initial binary search of b~z~ in an array of z coordinates. The fractional cascading aspect of the layered range tree is thus in respect to these z arrays.
...
Note that from ``3D with two onesided interval'' to ``3D with one onesided interval'' we turned a one-sided interval into a two-sided interval, at the same asymptotic query costs, but by paying a O(log n) factor space.
...
Data structure: Use a range tree, plus <<3d_range_tree>> for the last three dimensions.
