...
EXP (exponential):: (Decision) problems solvable in at most exponential time (2^n^).
...
Strongly NP-hard / NP-complete:: See <<strongly_NP_hard>>
...
We say that algorithm A is a _pseudo-polynomial-time algorithm_ for an integer-valued problem if its running time is O(p(Max-Int(x), |x|)), where p is some polynomial of two variables.  Simplified, an algorithm is a pseudo-poynomial time algorithm if its running time is polynomial either in the numeric value of input or in input size.

*to-do* Isn't then every algorithm in P a pseudo-polynomial algorithm? If yes, the wording `pseudo' is a bit special. It _truly_ is polynomial, but technically still also a _pseudo_-polynomial algorithm.  E.g. insertion sort O(n^2^): x = n \# e1 \# e2 # .... Running time depends now polynomically on the value of an input integer.

*to-do* is counting sort pseudo-polynomial?
...
[[strongly_NP_hard]]
An integer-valued problem U is called _strongly NP-hard_ if there exists a polynomial p such that the problem Value(p)-U is NP-hard.

Givn integer-valued problem U, and assuming P≠NP:
...
Let A be an consistent algorithm for an optimization problem, and OPT an optimal algorithm.  The _approximation ratio_ (or _approximation factor_ or _performance ratio_) of A on x is defined as Ratio~A~(x) = cost(A(x)) / cost(OPT(x)) for minimization problems, and the reciproce of it for maximization problems.

We say A is a _δ-approximation algorithm_ if Ratio~A~(x) ≤ δ ∀ feasible x.  In other words, δ is an upper bound on the worst case approximation ratio.

For any x, the _relative error_ of A is defined as ε~A~(x) = R~A~(x) - 1.

An approximation algorithm A is called a _polynomial-time approximation scheme_ (or _PTAS_) for optimization problem U if for ε > 0 and for all feasible x R~A~(x) ≤ 1 + ε and Time~A~(x, 1/ε) is bounded polynomial in |x| and unbounded in 1/ε.  A is called _fully polynomial-time approximation scheme_ (or _FPTAS_) if additionally Time~A~(x, 1/ε) is bounded polynimial also in 1/ε.  Intuition: PTAS lets you decrease approximation ratio by paying with computation time.  FPTAS guarantees you that computation time does not grow too quickly.

See also <<randomized_algs_for_optimization_problems>>, which could also have been a subchapter of ``Approximation algorithms'', but was made a subchapter of ``Randomized algorithms'' instead.  I thought I'll put concrete approximation algorithms close to the problem as always in this document, so the umbrella (sub)chapter for approximation agorithms is just this small one.  Online problems are relatively special compared to offline ones, and thus all online problems are within chapter online algorithms.
...
Another viewpoint, which, as written here, only becomes clear once you understand the van Emde Boas tree.  We want to build a data structure such that the time T(u) per operation can be given by the recurence relation T(u) = 1·T(√u) + O(1) = O(log log u), where the later equal sign is due to the master theorem.  Thus we can only afford one (the factor before T(√u)) further recursive call per recursion.
...
Time analysis for all operations: O(log log u) *to-do*
...
insert(x∈U) / delete(x∈S):  Find cluster / bottom tree where x should fit in or is.  Insert or delete x from that bottom tree.  Now we need to ensure that the bottom tree's size remains Θ(log u).  To be more specific, say we want the size to be within [1/2 log u, 2 log u].  If its too large, split in two.  If its too small, merge with a neighbor, which then might result in a split.  Of course each merge split also has to update the top tree.
...
Is a tree data structure solving the <<predecessor_problem>>.  Quick refresher: Given universe U of integers, word size w, maintain subset S of U. |U| = u ≤ 2^w^, |S| = n.

*to-do*

References:

- MIT course 6.851 Advanced Data Structures, spring 2012, lecture 12 ? *to-do*
...
[[ILP]]
_Integer linear programming_ (_ILP_) adds the additional constraint that numbers must be integers.  An even further restriction is _0-1 integer programming_ (_0-1 LP_) which demands that numbers are Boolean.
...
0-1 knapsack problem:: Now each item has a value v[i] ∈ ℕ^+^, and the goal
...
Overview of classification of randomized algorithms for solving decision problems and for computing functions. S(x) : 1 if A(x)=F(x), 0 otherwise. S for same.
...
[[randomized_algs_for_optimization_problems]]
=== Randomized Algorithms for Optimization Problems

See also <<approximation_algorithms>>.

Let A be a randomized algorithm for an optimization problem, let OPT be an optimal algorithm for the same problem, and let Ratio~A~(x) denote the approximation ratio of A on input x.  A is a __randomized E[δ]-approximation algorithm__ if for every feasible input x there exists an δ > 1 such that a) it returns a feasible solution and b) E[Ratio~A~(x)] ≤ δ.

With the same context, A is a __randomized δ-approximation algorithm__ if b) is replaced by Pr[Ratio~A~(x) ≤ δ] ≥ 1/2.  Note that such an algorithm can be repeated arbitrary many times to make Pr[Ratio~A~(x) ≤ δ] arbitrarily close to 1.

*to-do*: I forgot. Why exactly 1/2? Why not >0?

If A is a randomized E[δ]-approximation-algorithm for an optimization problem U, then A is a randomized (2E[δ])-approximation for U.  Note that an randomized E[δ]-approximation algorithm for U is not necessairly a randomized δ-approximation algorithm for U, and vice versa.
...
[[relax_LP_random_rounding]]
==== Relaxation to LP and random rounding

Many discrete optimization problem are NP-hard. Many of those are reducable to ILP.  We want to reduce the cost by trading in accuracy: We relax the ILP to an LP, solve that in polynomial time resulting in a fractional solution, and then convert that fractional solution to an integer one in polynomial time using some approximation strategy costing polynomial time.

The 1st step is to reduce the discrete optimization problem to <<ILP>> or O-1 LP.  This is often quite easy doable in a natural way.

The 2nd step is the _relaxation to LP_ and solving that LP, costing polynomial time, resulting in a fractional solution.  Relaxation to LP means removing the constraints which require integer or Boolean solutions respectively.  Note an optimal solution to this LP is as good or better than an optimum solution to the original problem U, because LP's constraints are a proper subset of U's constraints.

The 3rd step is converting the found optimal fraction solution to a reasonably good feasible solution to the original ILP or 0-1 LP.  One possible conversion strategy is random rounding which uses the fractional part as probability with which we round up opposed to round down.

Note that the fractional optimal solution to LP of step two can serve as an bound for the optimal solution of the original problem U.  An lower bound for minimization problems, and an upper bound for maximization problems respectively.

*to-do* What other convertion strategies than random rounding are there for step three? Answer: random rounding = irgendwie
...
==== A randomized approximation algorithm for the max-sat problem

A randomized approximation algorithm for the max-sat problem.  It makes use of the ``<<relax_LP_random_rounding,Relaxation to LP and random rounding>>'' technique.

*to-do*

*to-do* one chapter about sat problem and one chapter about the max-sat problem

References:

- Book "Design and Analysis of Randomized Algorithms", chapter "7.3 Random Rounding and MAX SAT"
...
Its not about approximation, see my lecture notes. So we calc prob of success. We don't calc approx ratio.
...
Given an online randomized algorithm, we can trivially turn it into an online algorithm with advice by overtaking the algorithm as is and the oracle provides the best possible advice string.  That is the best possible sequence of now no longer `random' bits.
...
If there is no online algorithm with advice that is c-competitive while using at most b(n) advice bits for ∏, then there is also no randomized nonline algorithm for ∏ that is c-competitive in expectation and that uses at most b(n) random bits. *to-do* why?
...
Definition: The input I = {w~1~, ..., w~n~} is a sequence of weights, where w~i~≤1.  A feasible solution is a set of indicies S ⊆ {1, ..., n} such that ∑~i∈S~w~i~≤1.  The goal is to minimize the sum.  In each timestep T~i~ the algorithm must make a final decision whether to pack w~i~ or not.
...
References to theorems are with respect to book ``An Introduction to Online Computation: Determinism, Randomization, Advice''. There you find also the proofs.

- Any deterministic algorithm (i.e 0 advice or random bits):

  * general: arbitrily high competitive ratio (theorem 6.1)

  * w~i~ ≤ β ≤ 1: gain ≥ 1-β or it's optimal (theorem 6.2)

- With advice:

  * One advice bit: strictly two competitive (theorem 6.6, <<KN_ONE>>)

  * Fewever than log~2~(n-1) advice bits: No o-Alg-w-A is better than strictly (2-ε)-competitive. (theorem 6.7)

  * O(log n) advice bits: Strictly (1+ε)-competitive  (theorem 6.8, <<KN_LOG>>)

  * Any optimal online algorithm with advice has to use at least n-1 advice bits (theorem 6.5)

  * n advice bits: (trivially) optimal (theorem 6.4)
...
opt=1 +XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXo--- trivial
      +++-----------+-+--------------++------>
...
No deterministic online algorithm for the simple knapsack problem is better than strictly 1/ε competitive, for any ε>0.
...
An online algorithm with advice using 1 advice bit. The advice bit tells whether all weights are smaller or equal than 1/2.  If they all are, a greedy strategy is used.  If there is at least one larger than 1/2, the algorithm waits until such an object is offered, packs it, and then does anything.

KN_ONE is strictly two comptetitive

Proof: Follows from theorem 6.2, see overview.
...
path = walk w. distinct vertices, except possibly first & last +
cycle = closed path +
...
An _Euler tour tree_ (_ET tree_) represents an undirected tree.  This chapter is tailored towards the use case of implementing the <<dynamic_connectivity>> ADT for undirected forests (i.e. for graph with no cycles).  Each of the represented undirected trees is stored using an Euler tour tree.  O(log n) time for all query and update operations and O(n) space. *to-do* If the represented forest is directed (i.e. rooted trees), while connectity queries still solve the problem of finding connected components, opposed to strongly connected components, then everything gets much simpler; namly the way MIT presents it, see references.

Note that the represented undirected tree might not be an binary tree, i.e. it might be a path. That's why we need a more complicated data structure than the naive tree: even if the represented trees are paths, we still want to provide O(log n) time for all operations.  Also the fact that it's an undirected tree means there is no obvious root.  This is a problem because connctivity problems are usually answered by choosing a representative node, and in an directed three that is typically the root..

The core idea of an ET tree is to store the Euler tour of the represented tree. Recall that strictly speaking a tree doesn't have an Euler Tour.  So figuratively double each edge in the represented tree for a Euler tour to become possible.

Aribitrarily pick any node of the represented undirected tree (component) as its root.  That node is  also called the tree's (components's) representative node, see also <<disjoint_set>>.  Imagine the Euler Tour as a curved line.  Make a mark on the line every time you visit a node, then pull that line straight.  The marks on the straight line define the leaves of a complete balanced binary tree.  I.e. the arbitrarily picked root of the represented tree corresponds to the leftmost and and the rightmost leaf in that balanced binary tree.  Note that the subtree rooted at v corresponds to a consecutive sequence in the Euler Tour, starting at v's first visit and ending at v's last visit.

data structures: *to-do*

_findrepresentative(v)_ / _findroot(v)_: Returns the representative, i.e. the arbitrily choosen root, of the represented tree containing v.  That corresponds to the minimum leaf of the ET tree containing v.  Alternatively you could also choose the maximum leaf.  Analysis: O(log n) time since we walk up ET Tree to its root, and then down to either minimum/leftmost (or maximum/rightmost) leaf.

_isconnected(v:vertex, w:vertex)_: findroot(v)==findroot(w).  Analysis: O(log n) since that's the complexity of findroot.

_cut(v:vertex)_:

Variant for directed represented trees: Cut the subtree rooted at v from the rest of the represented tree.  That corresponds to the contiguous sequence of ET tree leaves starting at firstvisit(v) and ending at lastvisit(v).  We find those two via the aformentioned pointers in the represented tree.  Split Euler-tour tree at firstvisit(v) and at lastvisit(v), see <<split_tree>>, and then concatenate the tree before firstvisit(v) and the tree after lastvisit(v), see <<concatenate_trees>>.  Analysis: Split uses O(log n), concatenate O(1) amortized, so O(log n) overall. *to-do* standford does it differently

_link(u:vertex, v:vertex)_:  As always, this operation is allowed to assume u and v are disconnected.  Make u and v the representative of their respective tree (component).  New Euler Tour is Eʹ = E~u~, E~v~, \{u\}.

Variant for directed represented trees:  Remains the same.

_setrepresentative(v:vertex)_ / _setroot(v:vertex)_: Split Euler tour E just befor an arbitrary visit to v, so E = E~1~, E~2~, where E~2~ beginns with \{v}. Remove E~1~'s first node resulting in Eʹ~1~. New Euler tour is Eʹ = E~2~, Eʹ~1~, \{v}.  Intuition: Look at concatenation building Eʹ and contemplate what E~2~ and Eʹ~1~ mean.  E~2~ means ``from (incl.) v upto (incl.) previous root v~old~'' and Eʹ~1~ means ``from just after previous root v~old~ to just before v''. *to-do* mnemonic indicies to E, at the cost of being longer
