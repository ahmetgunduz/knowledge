...
The term ``succinct data structures'' sadly is ambigous.  On one hand it is used as an umbrella term meaning space-efficient data structures.  On the other hand it designates a specific subgroup of space-efficient data structures, as defined in the following. Classification of data structures according to their space usage in bits (not words), where OPT is the information-theoretic optimum. 

_implicit_: OPT + O(1).  The ideal is ⌈OPT⌉, so the intend of O(1) is more that the OPT may be a fractional and we need to round up the next integer, rather than using O(1) bits for auxillary information.  Usually very hard, thus often not achieved.  Examples: heap, array.

_succinct_: OPT + o(OPT).  Most common type of space-efficient data structures.
...
- MIT course 6.851 Advanced Data Structures, lecture 10 Dictionaries: https://www.youtube.com/watch?v=Mf9Nn9PbGsE&index=10&t=0s&list=PLUl4u3cNGP61hsJNdULdudlRL493b-XZf

- Hacettepe University course BBN 402 Theory of Computation, Lecture 12 Hash Tables, https://web.cs.hacettepe.edu.tr/~ozkahya/classes/bbm402/Reading/Hashing.pdf
...
A _probe sequence_ is a permutation of all possible slots.  Obviously there are m! different probe sequences.  The hash function is generalized to take a further parameter: h(k,m,i) is the same as h(k,m), with the aditional parameter i, denothing the i-th probe. If h(k,m,0) returns a used slot, you try h(k,m,1) and so on.  An ideal such hash function belongs to the family of <<uniform_hash_functions>> (as opposed to just <<simple_uniform_hash_functions>>).  The definitions below make use of an _auxiliary hash function_ hʹ which is some `normal' hash function producing a single hash value.  _Primary clustering_ means that if there is a cluster of occupied slots and the initial position of a probe falls anywhere in the cluster, then the cluster size increases.  _Secondary clustiring_ is less severe, two probe sequences only have the same collision chain if their initial position is the same.
...
[[collision_resolution_with_perfect_]]
...
All nodes of the tree (inclusive leaves), are called internal nodes, and the `missing leaves', or in other words the null-pointer childs, are called external nodes.  Traverse this extended tree in level order, writing down a 1 for an internal node and a 0 for an external node, resulting in a bit vector.

Queries are based on <<rank>> and <<select>>:
...
Given a bit vector, determine number of ones upto index i.  Often used for <<succinct_data_structures>>. See also <<select>>
...
Given a bit vector, determine index of i-th 1.  Often used for <<succinct_data_structures>>. See also <<rank>>.
...
- Belongs to the family of <<simple_uniform_hash_functions>>. In practice, we often can only approximate.
...
[[simple_uniform_hash_functions]]
====  Familiy of simple uniform hash functions

*to-do* term ambiguity: uniform is apparently used for multiple things

[[ideal_hash_functions]]
A hash function is called _totally random_ (or _uniform_ (*to-do ambiguous*) or _ideal_ (*to-do really?*)) it satisfies the _simple uniform hashing assumption_ (_SUHA_), which is:
...
Note that the family of ideal hash functions and <<perfect_hash_functions>> are different things.  Perfect hashing guarantees no collisions, ideal hashing guarantees certain probabilities about collisions.
...
In _universal hashing_, at the creation of a given hash table, we select a hash function at random and independent from the keys that are actually going to be stored from a carefully designed set of hash functions.  This guarantees that no single input will always evoke worst-case behaviour.  A family H of hash functions is called a _universal familiy_ (or just _universal_) if

Pr~h∈H~[h(x) = h(y)] = O(1/m) for all h ∈ H and for all x ≠ y ∈ U.

*to-do* Somehow no. I apparently can't pick any h out of H and expext that the equation holds.

Alternatively: For for all x ≠ y ∈ U, the number of hash functions h∈H for which h(x) = h(y) is O(|H|/m).

Note that in other definitions, the rhs is ``≤ 1/m'' opposed to ``= O(1/m)''.

Motivation 1: Any fixed hash function maps multiple keys to the same slot.  Thus given a fixed hash function, a malicious adversary can always choose n keys that map to the same slot, yielding an average retrieval time of Θ(n).

Motivation 2: A true ideal hash function needs O(U log m) bits to implement, which is way too much. A hash function of a universal family (the weaker definition variant using big-oh) has weaker guearantees.

Examples of universal families:

- h(x) = ((ax) mod p) mod m, for a prime p and integer a such that 0<a<p. p should be larger than m, else the higher slots are unused. ax can also be vector dot product. Note that this is not exactly the same as the division method.  Is relatively expensive to compute due to the mod operations.

- h(x) = (ax) >> (log U − log m),  i.e. use log m high order bits of product ax.


[[perfect_hash_functions]]
==== Family of perfect hash functions

A _perfect hash function_ maps the set K of actual keys (as opposed to the universe U of all keys) to [m] with no collisions at all, i.e. it is an injective function.  It may be used to to implement a hash table with O(1) worst-case access time.

Note that perfect hash functions and ideal hash functions are different things, see respective paragraph in <<ideal_hash_functions>>.
...
Pr(⋀~1≤i≤k~h(x~i~)=t~i~) = O(1/m^k^) for all h ∈ H and for all distinct x~1~, ..., x~k~ ∈ U and for all (not necessarily distinct) t~1~, ..., t~k~ ∈ [m].
...
- With preprocessing: <<suffix_tree>>, <<suffix_array>>
...
Based on <<burrows_wheeler_transformation>>.
...
Trivia: The name stands for Full-text index in Minute space.
...
An online algorithm is _k-competitive_ if there exists a constant k such that its cost is at most k times the cost of an optimal offline algorithm, ignoring a constant offset α.  Intuitively it's a measurement of the online algorithm's quality.

--------------------------------------------------
online minimum problem: cost(ALG(I)) ≤ k·cost(OPT(I)) + α for all inputs I
online maximum problem: gain(OPT(I)) ≤ k·gain(ALG(I)) + α for all inputs I
--------------------------------------------------

The factor k is called the _competitive ratio_.  If the constant offset α is zero, the algorithm is said to be _strictly competitive_.  If k=1 and α=0, the online algorithm is _optimal_.  The notion of having the constant α allows an algorithm to achieve a good k for long inputs I, even if that algorithm is bad for short inputs I.  Using the paging problem as example, an _conservative algorithm_ has a cost of at most n  (n is the number of available pages) for any sequence of consecutive requests that involve (up to) n distinct pages. In other words, a conservative algorithm only updates its current hypothesis when making a mistake.
...
- LFDT (longest forward distance) or OPT (offline optimal) or MIN: Evict page to be used farthest in the future. A greedy algorithm. Proofably optimal. Cost +O(n/k)+ (n accesses, k pages). Proof: Only very k accesses there's an evict.
...
We like the ladder decomposition plus jump pointers approach due to its O(1) query time, but instead of O(n log n) space complexity, the nasty log n factor being due to the jump pointer tables, we would like O(n).  The main idea to reach O(n) space is to store jump tables only in few nodes,  and to make the bottom trees small enough such that there are less bottom tree types than actual bottom trees.  As a consequence we can store lookup tables per type, rather than use some data structure per actual bottom tree.
