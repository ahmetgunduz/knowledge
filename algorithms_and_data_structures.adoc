...
*to-do* move to chapter strings, just before FM Index

The _Burrows-Wheeler Transform_ (BWT) is a way of reversibly permuting the characters of a string T into another string BWT(T).

Given is a string T of length n over alphabet Σ; The last character of T is a special character $ ∉ Σ which is lexicographically smaller than any character in Σ.  The Burrow-Wheeler Transform of T, denoted BWT(T) (or just BWT) is a string of length n.  It's construction is described in the following.  Note that the BWT is the only thing which is actually stored.  The other introduced things are just for illustration.
...
------------------------------------------------------------
    01234567    BWM: Burrows-Wheeler Matrix
T = bananas$    F: first column of BWM, L: last column of BWM,
                BWT = L: Burrows wheeler transform of T,
                SA: auffix array, CAS: compressed suffix array,
                LF: LF Mapping

 Suffixes             BWM     
 given by SA        /------\            rank rankall
 = BWM up to $  row F      L  SA CAS LF R    a b . n . s .
 $              0   $bananas  7  7   7  0    0 0 0 0 0 1 0
 ananas$        1   ananas$b  1  1   4  0    0 1 0 0 0 1 0
 anas$          2   anas$ban  3  -   5  0    0 1 0 1 0 1 0
 as$            3   as$banan  5  -   6  1    0 1 0 2 0 1 0
 bananas$       4   bananas$  0  0   0  0    0 1 0 2 0 1 0
 anas$          5   nanas$ba  2  -   1  0    1 1 0 2 0 1 0
 nas$           6   nas$bana  4  4   2  1    2 1 0 2 0 1 0
 s$             7   s$banana  6  -   3  2    3 1 0 2 0 1 0
...
__Number of smaller letters / O array__: Is a building block optionally used in the following. For each letter c ∈ {$, Σ}, return the number of smaller letters in T.  Can be precomputed in O(n) time, storing the result in an array O of size |Σ|+1.  L is defined by O and vice versa, see also LF mapping.

__LF Mapping / LF array__: Is a building block used in the following.  Given row i, determine row j such that F[j] precedes L[i] in T.  j = LF[i] = O[L[i]] + R[i].  Rational: As descibed above, the ranks of characters remain in the same order, within F and L.  Note that F is given by sorting L, or in other words, by sorting the letters occuring in T and then repeat each letter by its occurence count.  Row j is then given by the row where character L[i] first appears in F (possibly precomputed in array O), plus the rank of character L[i] (possibly precomputed in array R).
...
__Construct BWT from T__: *to-do*

__Construct BWT from SA__: For i=0 to n(inclusive): if SA[i]>0 then BWT[i] = T[SA[i]-1], else BWT[i] = $.
...
__Compute SA[i] only given BWT__: *to-do*

__Search pattern P in T__: In the following, the range of selected rows defines the matches found so far. Initially, select the range of rows where F starts with the last character of P.  I.e. lastchar=P[|P|-1], from=O[lastchar], toincl=O[lastchar+1]-1.  Set i = |P| - 2. Iteratively narrow the range of rows by taking one more character of P into account, progressing from P's back towards P's front.  In each iteration, within the current range of rows, determine first and last occurence of P[i].  Let firstrow and lastrow denote that result.  We define later how to actually do that.  Via LF mapping, that defines the new range of rows: from=LF[firstrow], toincl=LF[lastrow].  The algorithm stops when the range of rows becomes empty, which means P does not occur in T.  Otherwise, after iterating over all P's characters, the remaining range of rows denotes the found matches.  The SA array tells us where in T to find each match.  *to-do* 1) profs idea that we overall there multiple rank select ops, and only one rank op remains at the end. maybe that only works with wavelet trees? 2) What's the difference now to use SA for indexing? After all, at the end we need SA also. 3) Merg with chapter FM Index.
...
- indexing, notably using little space: *to-do* see FM Index
...
- See also <<burrows_wheeler_transformation>>
