...
A _container_ is an assignment of resources.  Say a node has 8 cores and 64KB RAM, then each container might get assigned one core and 8GB RAM.  Each container can run a map task or reduce task or ApplicationMaster.
...
A processing model based on a DAG. A node in the DAG is an RDD (see below), an edge is a _transformation_.  To _create_ RDD s corresponding to root nodes in the DAG, we can create them from local or distributed fileystem, or from a process genereting it.  To make use of the RDD s, typically the ones corresponding to leaf nodes, there are so called _actions_. An action `materializes' an RDD.  We can e.g. dump it to local or distributed filesystem, or display it on the screen.  Evaluation is lazy; only when an action is invoked, the respective subgraph of the DAG is executed. Each action createas one _job_.  Multiple jobs can share the intermediate RDD s.

_Resilient distributed dataset_ (_RDD_): Collection of _values_. Each value can be anything.  Is partitioned, each partition can be on another machine.  The partitionboundary can be at any byte boundary. Thus when an RDD is for example stored on HDFS, we can take the HDFS blocks as partitioning.  A value can be an key-value pair, which case there are specific transformations and actions.  Note that potentially an RDD is distributed across multiple input machines.
...
Regarding a specific transformation, if the values of an RDD can be processed independently, they can be processed in parallel on multiple machines.  If they can't be processed independently, it needs shuffling as in MapReduce, which is an expensive operation.
...
==== Overview of general transformations
...
_flatMap_: Applies a function to every value in the input RDD. The output of the function is a collection. Overall each element of the output collection is aggregated into an RDD. Say the function produces an collection of 5 elements, then overall 5 RDD s are created.
...
==== Overview of key-value pair transformations

RDDs are not required to be lists of key-value pairs, but they can be, and some transformations make use of them.

_reduce by key_:  The function must be commutative and associative and the type of the returned pair must match the type of each input pair.  Expensive since it requires shuffling.


==== Overview of actions

_collect_:  Writes the RDD to some destination

_count_: Returns the number of values.

_count by value_: Returns a list of tuples (value, valuecount).

_reduce_ (as in MapReduce):
