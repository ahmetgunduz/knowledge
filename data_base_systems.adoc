...
**to-do** Why not make it in a more streaming like fashion (answer: it is done), i.e. make a stream of same key pairs to the reducer? Then while a mapper is still working, a reducer can already work on parts of its output. Similarily, why this hierarchical merging? Assistent asks for reference, I send an email.
...
Hadoop Version 2.0: YARN (yet another resource negotiator). The master is called YARN _ResourceManager_, the slaves are called YARN _NodeManager_.  The ResourceManager now only cares about scheduling.  Each NodeManager has a set of _containers_.  A container is an assignment of resources.  Say a node has 8 cores and 64KB RAM, then each container might get assigned one core and 8GB RAM.  Each container can run a map task or reduce task or ApplicationMaster. The monitoring is now done by an _ApplicationMaster_, which runs in a container.  Scalability is improved (v2 relative to v1), 10'000 nodes and 100'000 tasks, which is about the size of a data center.  Fully backwards compatible.
...
- _Capacity scheduler_: There's a set of hierarchical FIFO queues, taking user applications, each queue guarantees a certain capacity (resources, e.g. cores).  Each queue has strict ACLs that control which users can use the queue.  Idle capacity can be given to the most starved queues.
