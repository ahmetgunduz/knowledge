...
Let R(·) denote the expected risk, and let R̂(·) denote the empirical risk on the training set. 0≤δ≤1 is a parameter (called confidence), and ε is called precision. The following inequality delivers a way of knowing how large n should be, given a confidence and precision.

Pr[R̂(h) - R(h) ≤ ε] ≥ 1-δ ∀h∈𝓗 +
ε = √( ln(2|𝓗|/δ) / 2n ) +
R̂(h) - R(h) ≤ ε ⇔ R̂(h) [expected error] ≤ R(h) [empirical error] + ε [variance]
...
Based on the above inequality, we do large deviation: *to-do* what?!
