...
_test error_ R̂~test~(ĥ): Empirical risk using the test set and typically the fitted model ĥ. An unbiased estimator for the true risk, i.e. E~all train sets~[R̂~test~(ĥ)] < E[R(ĥ)], as long as the test data was not involved in any way in fitting ĥ.
...
For the k=1 case.  We want to find scalars z ∈ ℝ^n^ and a line given by w∈ℝ^d^ going through origin, such that x~i~ ≈ z~i~w.  That line is then a subspace of ℝ^d^ where we put the reduced versions of x~i~ on.

ŵ,ẑ = argmin~|w|=1,z~(∑~1≤i≤n~‖x~i~-z~i~w‖~2~²)

Assuming we know the optimal w, then the scalars are determined by z~i~=w^T^x~i~, since w is normalized by |w| = 1, and since z~i~ is just the base of the distance from the line to x~i~. Thus the problem becomes
...
For arbitrary k, we want to find a k-dimensional hyperplane, where we want to find an orthogonal matrix W ∈ ℝ^d⨯k^, each column having unit length, leading to the following optimization problem, called the _principal component analysis_ (_PCA_) problem:

Ŵ,ẑ = argmin~W,z~(∑~1≤i≤n~‖x~i~-Wz~i~‖~2~²)
