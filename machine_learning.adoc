...
_loss function_ l(ŷ,y) (or l(θ, x, y)): Measures how different the prediction ŷ = h~θ~(x) is from the true response y.  I.e. quantifies the loss of a single data point. *to-do* move / merge to correct more general place
...
_cost function_ Q (or L): Is more general than the loss function. It might be the sum of loss functions over a set of observations.
...
Prediction then is, as always with discriminant function:
