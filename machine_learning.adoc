...
Given a conditional distribution over labels p(y|x), a set 𝓐 of actions, and a cost function C:𝓨⨯𝓐→ℝ. Bayesian decision theory recommends to pick the action that minimizes the expected cost. This decision is called the _Bayesion optimal decision_.
...
- 𝓐 = {𝓨, "don't know" }. E.g. when we're really uncertain about the correct result, we admit that and return "don't know".

The cost function could e.g. be the 0-1 loss for classification (𝓐 = 𝓨).
...
_parameter_ θ, _parameter space_ Θ, _hypothesis_ h(x): *to-do*

_loss function_ l(ŷ,y) (or Q): Measures how different the prediction ŷ is from the true response y. *to-do* move / merge to correct more general place
...
_empirical risk_ (or _empirical error_) R̂~𝓓~(h) = E~(y,x)∈𝓓~[l(h(x),y)] = 1/n ∑~(y,x)∈𝓓~l(h(x),y). Since p(x,y) is often unknown, the expected risk R(h) cannot be computed. We can estimate the risk by averaging the loss on a sample data set 𝓓.
...
_training error_ R̂~train~(h): Empirical risk using the training set.

_test error_ R̂~test~(h): Empirical risk using the test set.
...
The _error function_ is the negative log of the likelihood function. Maximizing the likelihood is equivalent to minimizing the error.

_log likelihood_: *to-do*

_negative log likelihood_ (_NLL_): *to-do*

_prior (probability)_ p(θ): Probability of model paramters θ without having seen any data yet.
...
_maximum likelihood estimator_ (_MLE_) θ̂^MLE^ = argmax~θ~(p(𝓓|θ)): A frequentist estimator which sets the model parameters θ to the values that maximize the likelihood function p(𝓓|θ). This corresponds to choosing the model parameters θ for which the probability of the observed data set is maximized.
...
A _loss function_ (or _cost function_) L(Y, Ŷ) measures the error between Y and Ŷ = f̂(X). Typical choices are squared error (Y - Ŷ)² or absolute error |Y - Ŷ|.  Obviously we want to minimize the loss.
...
*to-do* other sources say shrinkage methods are about fighting overfitting
...
ŵ^MLE^ = argmin~w~(R̂(w)) +
R̂(w) = ∑~1≤i≤n~loss~logistic~(w)  [empirical risk] +
loss~logistic~(w) = log(1+exp(-y~i~w^T^x~i~))
...
Estimated Bayesion optimal decision under 0-1 loss: ŷ = a^∗^ = argmax~y~(p̂(y|x,ŵ)) = sign(ŵ^T^x). I.e. the action a^∗^ that minimizes the expected cost is the most likely class, where set of actions 𝓐 = {+1, -1} (i.e. the action is returning the guessed class) and cost function is the 0-1 loss.
...
Learning, variants: The stochastic gradient descent algorithms has variants. The inner loop of iterating over all observations can be replaced by uniformily at random pick only one observation. *to-do* this batch thing
...
We actually would like to minimize the number of missclassifications, i.e. use the 0/1 loss function, see below, but the resulting minimization optimization problem would be NP hard.  So instead we use a surrogate loss function, here the _perceptron loss function_ (or just _perceptron_), which is convex.  However when evaluationg the model, e.g. via cross-validation, use the original loss function, here the 0/1 loss function.
...
loss~0/1~(w, y~i~, x~i~) =  +
y~i~ notequal signʹ(w^T^x) = +
y~i~w^T^x < 0

Efficient computations despide feature space Φ(x) being higher dimensional than x: It can proven (representer theorem) that the optimal hyperplane ŵ is of the form ŵ = ∑α~i~y~i~x~i~.  Based on that idea, the learning of ŵ is simple (looking at stochastic gradient descent where an observation is choosen uniformily at random), even if Φ(x) has many more dimensions than x. Let t denote the index of the outer iteration, and j denote the index of the choosen observation.  Instead w = w + η~t~Φ(x)~j~t~j~, we do now α~j~ = α~j~ + η~t~.  However to compute ŷ now Φ(x)^T^Φ(xʹ) = k(x,xʹ), called kernel, must be efficiently computable. Indeed k(x,xʹ) is often more efficiently computable than Φ(x)^T^Φ(xʹ) as individual steps (first Φ(x), then transpose, etc.)
...
_kernel trick_ (or _kernelization_): Express problem s.t. it only depends on inner products. Replace those inner products by kernels.
...
Variation of the perceptron, but unlike the perceptron, is a _maximal margin classifier_, meaning that it maximizes the margin.

w = argmin~w~[loss~hinge~(w, y~i~, x~i~) + λ‖w‖~2~^2^] +
loss~hinge~(w, y~i~, x~i~) = max(0, 1-y~i~w^T^x~i~)

Gradient based algorithms can be used for this optimization problem (i.e. to find argmin~w~).

To find the regularization parameter λ, one can use cross validation.
...
[[lloyds_heuristic]]
=== _k-means algorithm_ (_Lloyd's heuristic_)
...
Imagine L columns (or layers) of units (or neurons or nodes) {u^(0)^, u^(1)^, ... u^(L)^}. The first column corresponds to the input x, i.e. u^(0)^ = x, the last column corresponds to the output y, i.e. y = u^(L)^.  As in linear regression / classification, the intercept / bias is absorbed into w, meaning that the true input x is extended by prepending an element which is always 1, resulting in the actually used input x.  The units of a given column depend on the units of the previous column the following way, where f~j~(·) is a (possibly nonlinear) _activation function_, and w~j~^(l)^ is the parameter vector of the l-th (transformation-)layer for the j-th unit in the l-th column.  The units in the columns between the first (input) and last (output) column are called _hidden units_.

u~j~^(l)^ = f~j~((w~j~^(l)^)^T^u^(l-1)^)

Recall that is similar to what linear regression / classification does with its y = f(w^T^Φ(x)), where Φ(x) = (Φ~1~(x), ...,Φ~M~(x)), where Φ~i~(·) are possibly non-linear basis functions.  In the neuranal network, Φ is now given by the previous layer.

In case of a single hidden layer network, having k units in the hidden layer:

h(x) = ∑~1≤j≤k~wʹ~j~f~j~(w~j~^T^x)

There is no consensus what the n in a n-layer neuronal network counts. 1) The number of transformations 2) the number of layers of hidden units 3) the number of layers of units (input, output and hidden).

Neuronal networks are _universal adaptors_. For example, a two-layer (transformations) with linear output and non-linear hidden unit activation functions can approximate any continuous function on a compact input domain to arbitrary accuracy, provided sufficiently many hidden units.
