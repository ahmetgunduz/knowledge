...
The name `generative model' comes from that the p(x|y) which we estimate can be seen as generating a feature vector given a label.  That can be usefull to impute missing data.
...
μ̂~y,j~ = mean(all x~i~^(j)^ where y~i~=y) [by MLE] +
σ̂²~y,j~ = var(analogous) [by MLE]
...
p(x~i~|μ~j~,Σ~j~) = 𝓝~d~(x;μ~j~,Σ~j~) +
p(x|w,μ,Σ) = ∑~1≤j≤K~w~j~p(x~i~|μ~j~,Σ~j~) +
w = (w~1~, ..., w~K~) +
